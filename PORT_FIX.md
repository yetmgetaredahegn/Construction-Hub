...
rm -f dist/Construction-Hub.war
bash build.sh

# If using system Tomcat:
sudo cp dist/Construction-Hub.war /var/lib/tomcat10/webapps/
sudo systemctl restart tomcat10
# Then access: http://localhost:8080/Construction-Hub/

# OR if using custom Tomcat:
./run.sh
# Then access: http://localhost:8081/Construction-Hub/
```
